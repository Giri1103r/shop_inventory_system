...
class FloorStretcherFiles extends Model
{
    ...
    public function signatureUpload($type,$id)
    {
        try {
            ...
            if ($request->has('signature_image')) {
                ...
                $upload_path = 'public/uploads/inspection/ohc/floor_stretcher/signatureupload';
    ...
    {
        return $this->where('inspection_id',$id)->where('type',$type)->where('status',1)->where('trash','NO')->first();
    }
}
