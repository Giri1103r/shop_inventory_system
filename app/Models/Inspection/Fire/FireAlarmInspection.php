...
class FireAlarmInspection extends Model
{
    ...
    public function list()
    {
        ...
        if (CheckUserRole(ROLE_SUPERADMIN) || CheckUserRole(ROLE_EHS_OFFICER) || CheckUserRole(ROLE_L1_MANAGER) || CheckUserRole(ROLE_L2_MANAGER)) {
            ...
            $query->where('inspection_fire_fire_alarm.created_by', Auth::id());
        }
    ...
    public function listApi()
    {
        ...
        $query = $this->select('inspection_fire_fire_alarm.*', 'inspection_shift_option.*', 'masters_unit.*', 'masters_location.*', 'inspection_frequency_option.*', 'inspection_fire_fire_alarm.id as inspection_id', 'inspection_static_docno.*')
            ->leftJoin('masters_location', 'inspection_fire_fire_alarm.location', '=', 'masters_location.id')
            ->leftJoin('inspection_shift_option', 'inspection_fire_fire_alarm.shift', '=', 'inspection_shift_option.id')
            ->leftJoin('masters_unit', 'inspection_fire_fire_alarm.unit', '=', 'masters_unit.id')
        ...
        $paginatedData = $query->orderBy('inspection_fire_fire_alarm.id', 'DESC')->paginate($perPage);
