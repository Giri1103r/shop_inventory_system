...
class FireSignatureUpload extends Model
{
    ...
    public function CheckedBySignatureApi($id, $type)
    {
        $request = request();
        $upload_path = 'public/uploads/inspection/fire/hooter_inspection/' . $id.'/signature_upload';

        if ($request->has('signature_image') && is_array($request->signature_image)) {
            $base64File = $request->file('signature_image');
            if (!empty($base64File)) {
                    $extension = null;
                    $fileType = null;
    ...
    public function observationFollowUp($type, $id)
    {
        try {
            $request = Request();
            $this->where('inspection_id', decryptId($id))->update(['trash' => 'YES'], ['status' => '0']);
            $file = $request->file('signature_image');
            if ($request->has('signature_image')) {
                $image = $request->file('signature_image');
                $upload_path = 'public/uploads/inspection/observation_followup/signatureupload';
