...
class FireModularInspection extends Model
{

    protected $table = 'inspection_fire_modular_inspection';
    ...
    public function list()
    {
        $request = request();
        $search = '';
        $query = $this->select('inspection_fire_modular_inspection.*', 'inspection_shift_option.*', 'masters_unit.*', 'masters_location.*', 'inspection_frequency_option.*', 'inspection_fire_modular_inspection.id as fire_detector_id')
        ...
        if (isset($request->frequency) && $request->frequency) {
            $query = $query->where('inspection_fire_modular_inspection.frequency', 'LIKE', '%' . decryptId($request->frequency) . '%');
        }
        if (isset($request->unit) && $request->unit) {
            $query = $query->where('inspection_fire_modular_inspection.unit', 'LIKE', '%' . decryptId($request->unit) . '%');
        }
        ...
        if (isset($request->inspection_status) && $request->inspection_status) {
         ...
         if ($request->has('from_date') && !empty($request->from_date)) {
            ...
            $query->where('inspection_fire_modular_inspection.created_at', '>=', $startDate);
        }
        if ($request->has('to_date') && !empty($request->to_date)) {
            $endDate = Carbon::createFromFormat('d-m-Y', $request->to_date)->endOfDay()->format('Y-m-d H:i:s');
            $query->where('inspection_fire_modular_inspection.created_at', '<=', $endDate);
        }
        if ($request->has('from_date') && !empty($request->from_date) && $request->has('to_date') && !empty($request->to_date)) {
            $startDate = Carbon::createFromFormat('d-m-Y', $request->from_date)->startOfDay()->format('Y-m-d H:i:s');
            $endDate = Carbon::createFromFormat('d-m-Y', $request->to_date)->endOfDay()->format('Y-m-d H:i:s');
            $query->whereBetween('inspection_fire_modular_inspection.created_at', [$startDate, $endDate]);
        }
        ...
        if (isset($request->order) && count($request->order) > 0) {
            ...
            switch ($columnName) {
                case "inspection_status":
                    $query = $query->orderBy('inspection_fire_modular_inspection.inspection_status', $columnorder);
                    break;
                case "created_by":
                    $query = $query->orderBy('inspection_fire_modular_inspection.created_by', $columnorder);
                    break;
                case "created_date":
                    $query = $query->orderBy('inspection_fire_modular_inspection.created_at', $columnorder);
                    break;
                default:
                    $query = $query->orderBy('inspection_fire_modular_inspection.id', 'DESC');
                    break;
            }
        }
    ...
    public function exportdata()
    {
        $request = request();
        $search = '';
        $query = $this->select('inspection_fire_modular_inspection.*', 'inspection_shift_option.*', 'inspection_fire_modular_inspection_details.*', 'masters_unit.*', 'masters_location.*', 'inspection_frequency_option.*', 'inspection_static_docno.*', 'inspection_fire_modular_inspection.id as fire_id', 'inspection_fire_modular_inspection.created_by as checked_by', 'inspection_fire_modular_inspection.updated_by as verified_by')
            ->leftJoin('masters_location', 'inspection_fire_modular_inspection.location', '=', 'masters_location.id')
            ->leftJoin('inspection_shift_option', 'inspection_fire_modular_inspection.shift', '=', 'inspection_shift_option.id')
            ->leftJoin('masters_unit', 'inspection_fire_modular_inspection.unit', '=', 'masters_unit.id')
            ->leftJoin('inspection_static_docno', 'inspection_fire_modular_inspection.document_reference_id', '=', 'inspection_static_docno.id')
            ->leftJoin('inspection_fire_modular_inspection_details', 'inspection_fire_modular_inspection.id', '=', 'inspection_fire_modular_inspection_details.inspection_id')
            ->leftJoin('inspection_frequency_option', 'inspection_fire_modular_inspection.frequency', '=', 'inspection_frequency_option.id');
        ...
        $query->orderBy('inspection_fire_modular_inspection.id', 'DESC');
    ...
    {
        static::addGlobalScope(new TrashScope('inspection_fire_modular_inspection'));
    }
}
