...
class SprinklarSystemInspection extends Model
{
    protected $table = 'inspection_fire_sprinklar_system';
    ...
    public function list()
    {
        $request = request();
        $search = '';
        $query = $this->select('inspection_fire_sprinklar_system.*', 'inspection_shift_option.*', 'masters_unit.*', 'masters_location.*', 'inspection_frequency_option.*', 'inspection_fire_sprinklar_system.id as inspection_id')
            ->leftJoin('masters_location', 'inspection_fire_sprinklar_system.location', '=', 'masters_location.id')
        ...
        if (CheckUserRole(ROLE_SUPERADMIN) || CheckUserRole(ROLE_EHS_OFFICER) || CheckUserRole(ROLE_L1_MANAGER) || CheckUserRole(ROLE_L2_MANAGER)) {
            ...
            $query->where('inspection_fire_sprinklar_system.created_by', Auth::id());
        }
        ...
        if (isset($request->location) && $request->location) {
            $query = $query->where('inspection_fire_sprinklar_system.location', 'LIKE', '%' . decryptId($request->location) . '%');
        }
        ...
        if (isset($request->unit) && $request->unit) {
            $query = $query->where('inspection_fire_sprinklar_system.unit', 'LIKE', '%' . decryptId($request->unit) . '%');
        }
        if (isset($request->shift) && $request->shift) {
            $query = $query->where('inspection_fire_sprinklar_system.shift', 'LIKE', '%' . decryptId($request->shift) . '%');
        }
        if (isset($request->location) && $request->location) {
            $query = $query->where('inspection_fire_sprinklar_system.location', 'LIKE', '%' . decryptId($request->location) . '%');
        }
        ...
        if ($request->has('from_date') && !empty($request->from_date)) {
            ...
            $query->where('inspection_fire_sprinklar_system.created_at', '>=', $startDate);
        }
        if ($request->has('to_date') && !empty($request->to_date)) {
            $endDate = Carbon::createFromFormat('d-m-Y', $request->to_date)->endOfDay()->format('Y-m-d H:i:s');
            $query->where('inspection_fire_sprinklar_system.created_at', '<=', $endDate);
        }
        if ($request->has('from_date') && !empty($request->from_date) && $request->has('to_date') && !empty($request->to_date)) {
            $startDate = Carbon::createFromFormat('d-m-Y', $request->from_date)->startOfDay()->format('Y-m-d H:i:s');
            $endDate = Carbon::createFromFormat('d-m-Y', $request->to_date)->endOfDay()->format('Y-m-d H:i:s');
            $query->whereBetween('inspection_fire_sprinklar_system.created_at', [$startDate, $endDate]);
        }


        if (isset($request->inspection_status) && $request->inspection_status) {
            $query = $query->where('inspection_fire_sprinklar_system.inspection_status', decryptId($request->inspection_status));
        }
        ...
        if (isset($request->order) && count($request->order) > 0) {
            ...
            switch ($columnName) {
                case "inspection_status":
                    $query = $query->orderBy('inspection_fire_sprinklar_system.inspection_status', $columnorder);
                    break;
                case "created_by":
                    $query = $query->orderBy('inspection_fire_sprinklar_system.created_by', $columnorder);
                    break;
                case "created_date":
                    $query = $query->orderBy('inspection_fire_sprinklar_system.created_at', $columnorder);
                    break;
                default:
                    $query = $query->orderBy('inspection_fire_sprinklar_system.id', 'DESC');
                    break;
            }
        }
    ...
    public function listApi()
    {
        ...
        $query = $this->select('inspection_fire_sprinklar_system.*', 'inspection_shift_option.*', 'masters_unit.*', 'masters_location.*', 'inspection_frequency_option.*', 'inspection_fire_sprinklar_system.id as inspection_id', 'inspection_static_docno.*')
            ->leftJoin('masters_location', 'inspection_fire_sprinklar_system.location', '=', 'masters_location.id')
            ->leftJoin('inspection_shift_option', 'inspection_fire_sprinklar_system.shift', '=', 'inspection_shift_option.id')
            ->leftJoin('masters_unit', 'inspection_fire_sprinklar_system.unit', '=', 'masters_unit.id')
        ...
        $paginatedData = $query->orderBy('inspection_fire_sprinklar_system.id', 'DESC')->paginate($perPage);
    ...
    public function exportdata()
    {
        $request = request();
        $search = '';
        $query = $this->select('inspection_fire_sprinklar_system.*', 'inspection_shift_option.*', 'masters_unit.*', 'masters_location.*', 'inspection_frequency_option.*', 'inspection_fire_sprinklar_system.id as fire_id', 'inspection_fire_sprinklar_system.created_by as checked_by', 'inspection_fire_sprinklar_system_details.*')
            ->leftJoin('masters_location', 'inspection_fire_sprinklar_system.location', '=', 'masters_location.id')
            ->leftJoin('inspection_shift_option', 'inspection_fire_sprinklar_system.shift', '=', 'inspection_shift_option.id')
            ->leftJoin('masters_unit', 'inspection_fire_sprinklar_system.unit', '=', 'masters_unit.id')
            ->leftJoin('inspection_frequency_option', 'inspection_fire_sprinklar_system.frequency', '=', 'inspection_frequency_option.id')
        ...
        $query->orderBy('inspection_fire_sprinklar_system.id', 'DESC');
    ...
    {
        static::addGlobalScope(new TrashScope('inspection_fire_sprinklar_system'));
    }
}
