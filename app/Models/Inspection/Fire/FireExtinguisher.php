...
{
    protected $table = 'inspection_fire_fire_extinguisher';
    ...
    public function list()
    {
        ...
        if ($request->has('from_date') && !empty($request->from_date)) {
            ...
            $query->where('inspection_fire_fire_extinguisher.created_at', '>=', $startDate);
        }
        if ($request->has('to_date') && !empty($request->to_date)) {
            $endDate = Carbon::createFromFormat('d-m-Y', $request->to_date)->endOfDay()->format('Y-m-d H:i:s');
            $query->where('inspection_fire_fire_extinguisher.created_at', '<=', $endDate);
        }
        if ($request->has('from_date') && !empty($request->from_date) && $request->has('to_date') && !empty($request->to_date)) {
            $startDate = Carbon::createFromFormat('d-m-Y', $request->from_date)->startOfDay()->format('Y-m-d H:i:s');
            $endDate = Carbon::createFromFormat('d-m-Y', $request->to_date)->endOfDay()->format('Y-m-d H:i:s');
            $query->whereBetween('inspection_fire_fire_extinguisher.created_at', [$startDate, $endDate]);
        }
        ...
        if (isset($request->order) && count($request->order) > 0) {
            ...
            switch ($columnName) {
                case "inspection_status":
                    ...
                    break;
                case "created_by":
                    $query = $query->orderBy('inspection_fire_fire_extinguisher.created_by', $columnorder);
                    break;
                case "created_date":
                    $query = $query->orderBy('inspection_fire_fire_extinguisher.created_at', $columnorder);
                    break;
                default:
                    $query = $query->orderBy('inspection_fire_fire_extinguisher.id', 'DESC');
                    break;
            }
        }
    ...
    public function listApi()
    {
        ...
        $query = $this->select('inspection_fire_fire_extinguisher.*', 'inspection_shift_option.*', 'masters_unit.*', 'masters_location.*', 'inspection_frequency_option.*', 'inspection_fire_fire_extinguisher.id as inspection_id', 'inspection_static_docno.*')
            ->leftJoin('masters_location', 'inspection_fire_fire_extinguisher.location', '=', 'masters_location.id')
            ->leftJoin('inspection_shift_option', 'inspection_fire_fire_extinguisher.shift', '=', 'inspection_shift_option.id')
            ->leftJoin('masters_unit', 'inspection_fire_fire_extinguisher.unit', '=', 'masters_unit.id')
        ...
        $paginatedData = $query->orderBy('inspection_fire_fire_extinguisher.id', 'DESC')->paginate($perPage);
    ...
    {
        static::addGlobalScope(new TrashScope('inspection_fire_fire_extinguisher'));
    }
}
