...
class HoseReelHoseInspection extends Model
{
    protected $table = 'inspection_fire_hose_reel';
    ...
    public function list()
    {
        $request = request();
        $search = '';
        $query = $this->select('inspection_fire_hose_reel.*', 'inspection_shift_option.*', 'masters_unit.*', 'masters_location.*', 'inspection_frequency_option.*', 'inspection_fire_hose_reel.id as inspection_id')
            ->leftJoin('masters_location', 'inspection_fire_hose_reel.location', '=', 'masters_location.id')
            ->leftJoin('inspection_shift_option', 'inspection_fire_hose_reel.shift', '=', 'inspection_shift_option.id')
            ->leftJoin('masters_unit', 'inspection_fire_hose_reel.unit', '=', 'masters_unit.id')
            ->leftJoin('inspection_frequency_option', 'inspection_fire_hose_reel.frequency', '=', 'inspection_frequency_option.id');
        ...
        if (isset($request->unit) && $request->unit) {
            $query = $query->where('inspection_fire_hose_reel.unit', 'LIKE', '%' . decryptId($request->unit) . '%');
        }
        if (isset($request->shift) && $request->shift) {
            $query = $query->where('inspection_fire_hose_reel.shift', 'LIKE', '%' . decryptId($request->shift) . '%');
        }
        if (isset($request->location) && $request->location) {
            $query = $query->where('inspection_fire_hose_reel.location', 'LIKE', '%' . decryptId($request->location) . '%');
        }
        ...
        if ($request->has('from_date') && !empty($request->from_date)) {
            ...
            $query->where('inspection_fire_hose_reel.created_at', '>=', $startDate);
        }
        if ($request->has('to_date') && !empty($request->to_date)) {
            $endDate = Carbon::createFromFormat('d-m-Y', $request->to_date)->endOfDay()->format('Y-m-d H:i:s');
            $query->where('inspection_fire_hose_reel.created_at', '<=', $endDate);
        }
        if ($request->has('from_date') && !empty($request->from_date) && $request->has('to_date') && !empty($request->to_date)) {
            $startDate = Carbon::createFromFormat('d-m-Y', $request->from_date)->startOfDay()->format('Y-m-d H:i:s');
            $endDate = Carbon::createFromFormat('d-m-Y', $request->to_date)->endOfDay()->format('Y-m-d H:i:s');
            $query->whereBetween('inspection_fire_hose_reel.created_at', [$startDate, $endDate]);
        }
        ...
        if (isset($request->order) && count($request->order) > 0) {
            ...
            switch ($columnName) {
                case "inspection_status":
                    $query = $query->orderBy('inspection_fire_hose_reel.inspection_status', $columnorder);
                    break;
                case "created_by":
                    $query = $query->orderBy('inspection_fire_hose_reel.created_by', $columnorder);
                    break;
                case "created_date":
                    $query = $query->orderBy('inspection_fire_hose_reel.created_at', $columnorder);
                    break;
                default:
                    $query = $query->orderBy('inspection_fire_hose_reel.id', 'DESC');
                    break;
            }
        }
    ...
    public function listApi()
    {
        ...
        $query = $this->select('inspection_fire_hose_reel.*', 'inspection_shift_option.*', 'masters_unit.*', 'masters_location.*', 'inspection_frequency_option.*', 'inspection_fire_hose_reel.id as inspection_id', 'inspection_static_docno.*')
            ->leftJoin('masters_location', 'inspection_fire_hose_reel.location', '=', 'masters_location.id')
            ->leftJoin('inspection_shift_option', 'inspection_fire_hose_reel.shift', '=', 'inspection_shift_option.id')
            ->leftJoin('masters_unit', 'inspection_fire_hose_reel.unit', '=', 'masters_unit.id')
            ->leftJoin('inspection_static_docno', 'inspection_fire_hose_reel.document_reference_id', '=', 'inspection_static_docno.id')
            ->leftJoin('inspection_frequency_option', 'inspection_fire_hose_reel.frequency', '=', 'inspection_frequency_option.id');
        ...
        $paginatedData = $query->orderBy('inspection_fire_hose_reel.id', 'DESC')->paginate($perPage);
    ...
    public function exportdata()
    {
        $request = request();
        $search = '';
        $query = $this->select('inspection_fire_hose_reel.*', 'inspection_shift_option.*', 'masters_unit.*', 'masters_location.*', 'inspection_frequency_option.*', 'inspection_fire_hose_reel.id as fire_id', 'inspection_fire_hose_reel_details.*', 'inspection_fire_hose_reel.created_by as checked_by')
            ->leftJoin('masters_location', 'inspection_fire_hose_reel.location', '=', 'masters_location.id')
            ->leftJoin('inspection_shift_option', 'inspection_fire_hose_reel.shift', '=', 'inspection_shift_option.id')
            ->leftJoin('masters_unit', 'inspection_fire_hose_reel.unit', '=', 'masters_unit.id')
            ->leftJoin('inspection_static_docno', 'inspection_fire_hose_reel.document_reference_id', '=', 'inspection_static_docno.id')
            ->leftJoin('inspection_fire_hose_reel_details', 'inspection_fire_hose_reel.id', '=', 'inspection_fire_hose_reel_details.inspection_id')
            ->leftJoin('inspection_frequency_option', 'inspection_fire_hose_reel.frequency', '=', 'inspection_frequency_option.id');
        ...
        $query->orderBy('inspection_fire_hose_reel.id', 'DESC');
    ...
    {
        static::addGlobalScope(new TrashScope('inspection_fire_hose_reel'));
    }
}
