...
class IsolationValve extends Model
{
    ...
    public function list()
    {
        ...
        if (CheckUserRole(ROLE_SUPERADMIN) || CheckUserRole(ROLE_EHS_OFFICER) || CheckUserRole(ROLE_L1_MANAGER) || CheckUserRole(ROLE_L2_MANAGER)) {
            ...
            $query->where('inspection_fire_isolation_valve.created_by', Auth::id());
        }
        ...
        if (isset($request->inspection_date) && $request->inspection_date) {
            $query = $query->where('inspection_fire_isolation_valve.date_of_inspection', 'LIKE', '%' . DBdateformat($request->inspection_date) . '%');
        }
        ...
        if ($request->has('to_date') && !empty($request->to_date)) {
            ...
            $query->where('inspection_fire_isolation_valve.created_at', '<=', $endDate);
        }
        if ($request->has('from_date') && !empty($request->from_date) && $request->has('to_date') && !empty($request->to_date)) {
            $startDate = Carbon::createFromFormat('d-m-Y', $request->from_date)->startOfDay()->format('Y-m-d H:i:s');
            $endDate = Carbon::createFromFormat('d-m-Y', $request->to_date)->endOfDay()->format('Y-m-d H:i:s');
            $query->whereBetween('inspection_fire_isolation_valve.created_at', [$startDate, $endDate]);
        }
    ...
    public function listApi()
    {
        ...
        $query = $this->select('inspection_fire_isolation_valve.*', 'inspection_shift_option.*', 'masters_unit.*', 'masters_location.*', 'inspection_frequency_option.*', 'inspection_fire_isolation_valve.id as inspection_id', 'inspection_static_docno.*')
            ->leftJoin('masters_location', 'inspection_fire_isolation_valve.location', '=', 'masters_location.id')
            ->leftJoin('inspection_shift_option', 'inspection_fire_isolation_valve.shift', '=', 'inspection_shift_option.id')
            ->leftJoin('masters_unit', 'inspection_fire_isolation_valve.unit', '=', 'masters_unit.id')
        ...
        $paginatedData = $query->orderBy('inspection_fire_isolation_valve.id', 'DESC')->paginate($perPage);
    ...
    public function exportdata()
    {
        ...
        $query->orderBy('inspection_fire_isolation_valve.id', 'DESC');
