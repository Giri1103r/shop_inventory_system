...
class CoTypeFireExtinguisher extends Model
{
    protected $table = 'inspection_co_type_fire_extinguisher';
    ...
    public function list()
    {
        ...
        if (CheckUserRole(ROLE_SUPERADMIN) || CheckUserRole(ROLE_EHS_OFFICER) || CheckUserRole(ROLE_L1_MANAGER) || CheckUserRole(ROLE_L2_MANAGER)) {
            ...
            $query->where('inspection_co_type_fire_extinguisher.created_by', Auth::id());
        }
        ...
        if (isset($request->unit) && $request->unit) {
            $query = $query->where('inspection_co_type_fire_extinguisher.unit', 'LIKE', '%' . decryptId($request->unit) . '%');
        }
        if (isset($request->shift) && $request->shift) {
            $query = $query->where('inspection_co_type_fire_extinguisher.shift', 'LIKE', '%' . decryptId($request->shift) . '%');
        }
        if (isset($request->location) && $request->location) {
            $query = $query->where('inspection_co_type_fire_extinguisher.location', 'LIKE', '%' . decryptId($request->location) . '%');
        }
        ...
        if ($request->has('to_date') && !empty($request->to_date)) {
            ...
            $query->where('inspection_co_type_fire_extinguisher.created_at', '<=', $endDate);
        }
        if ($request->has('from_date') && !empty($request->from_date) && $request->has('to_date') && !empty($request->to_date)) {
            $startDate = Carbon::createFromFormat('d-m-Y', $request->from_date)->startOfDay()->format('Y-m-d H:i:s');
            $endDate = Carbon::createFromFormat('d-m-Y', $request->to_date)->endOfDay()->format('Y-m-d H:i:s');
            $query->whereBetween('inspection_co_type_fire_extinguisher.created_at', [$startDate, $endDate]);
        }
        ...
        if (isset($request->order) && count($request->order) > 0) {
            ...
            switch ($columnName) {
                case "inspection_status":
                    $query = $query->orderBy('inspection_co_type_fire_extinguisher.inspection_status', $columnorder);
                    break;
                case "created_by":
                    $query = $query->orderBy('inspection_co_type_fire_extinguisher.created_by', $columnorder);
                    break;
                case "created_date":
                    $query = $query->orderBy('inspection_co_type_fire_extinguisher.created_at', $columnorder);
                    break;
                default:
                    $query = $query->orderBy('inspection_co_type_fire_extinguisher.id', 'DESC');
                    break;
            }
        }
