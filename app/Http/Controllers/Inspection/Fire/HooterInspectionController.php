...
class HooterInspectionController extends Controller
{
                ...
                try {
                    ...
                    $datatables = DataTables::of($data['data'])
                        ...
                        ->addColumn('action', function ($row) {
                            $btn = '';
                            $btn = '<a href="' . admin_url('fire/hooter-inspection/view/' . encryptId($row->inspection_id)) . '"   class="view-icon me-1" title="' . __('common.view') . '"><i class="fa-solid fa-eye"></i></a> ';
                            if ($row->inspection_status == WAITING_FOR_EHS_OFFICER_VERIFICATION && (CheckUserRole(ROLE_EHS_OFFICER) || isAdmin())) {
                                $btn .= '<a href="' . admin_url('fire/hooter-inspection/verification/' . encryptId($row->inspection_id)) . '/ehs" class="me-1" title="' . __('inspection.ehs_officer_verify') . '"><i class="fa-solid fa-check-to-slot text-success"></i></a> ';
                            }
                            if (($row->inspection_status == WAITING_FOR_CAPA_ACTION || $row->inspection_status == L2_MANAGER_REJECTED || $row->inspection_status == EHS_OFFICER_REJECTED || $row->inspection_status == L1_MANAGER_REJECTED) && (CheckUserRole(ROLE_FIRE_ASSOCIATES) || isAdmin())) {
                                $btn .= '<a href="' . admin_url('fire/hooter-inspection/verification/' . encryptId($row->inspection_id)) . '/capa" class="me-1" title="' . __('inspection.capa_action') . '"><i class="fa-solid fa-check-to-slot text-success"></i></a> ';
                            }
                            if ($row->inspection_status == WAITING_FOR_CAPA_VERIFICATION && (CheckUserRole(ROLE_EHS_OFFICER) || isAdmin())) {
                                $btn .= '<a href="' . admin_url('fire/hooter-inspection/verification/' . encryptId($row->inspection_id)) . '/ehsVerify" class="me-1" title="' . __('inspection.ehs_officer_verify') . '"><i class="fa-solid fa-check-to-slot text-success"></i></a> ';
                            }
                            if ($row->inspection_status == WAITING_FOR_L1_VERIFICATION && (CheckUserRole(ROLE_L1_MANAGER) || isAdmin())) {
                                $btn .= '<a href="' . admin_url('fire/hooter-inspection/verification/' . encryptId($row->inspection_id)) . '/level-one-manager" class="me-1" title="' . __('inspection.l1_manager_verify') . '"><i class="fa-solid fa-check-to-slot text-success"></i></a> ';
                            }
                            if ($row->inspection_status == WAITING_FOR_L2_VERIFICATION && (CheckUserRole(ROLE_L2_MANAGER) || isAdmin())) {
                                $btn .= '<a href="' . admin_url('fire/hooter-inspection/verification/' . encryptId($row->inspection_id)) . '/level-two-manager" class="me-1" title="' . __('inspection.l2_manager_verify') . '"><i class="fa-solid fa-check-to-slot text-success"></i></a> ';
                            }
    ...
    public function ExportExcel(Request $request)
    {
        try {
            ...
            foreach ($allData as $groupedDetails) {
                ...
                $prepared_by_signature = GetFireSignature($inspection_detail->checked_by, $inspection_detail->fire_id, HOOTER_INSPECTION);
                $verified_by_signature = GetFireSignature($inspection_detail->verified_by, $inspection_detail->fire_id, HOOTER_INSPECTION);
                $approved_by_signature = GetFireSignature($inspection_detail->approved_by, $inspection_detail->fire_id, HOOTER_INSPECTION);
                ...
                $sheet->setCellValue("C{$titleRow}", "HOOTER  INSPECTION CHECKLIST");
                ...
                $sheet->setCellValue("E" . ($headerStart + 1), "BLINKING LIGHT");
                $sheet->setCellValue("F" . ($headerStart + 1), "CONNECTION");
                $sheet->setCellValue("G" . ($headerStart + 1), "AUDIOBILITY");
                $sheet->setCellValue("H" . ($headerStart + 1), "CONDITION OF THE HOOTER");
                ...
                foreach ($groupedDetails as $detail) {
                    ...
                    $sheet->setCellValue("D{$dataRow}", $statusMap[$detail['quantity'] ?? ''] ?? $detail['quantity']);
                    $sheet->setCellValue("E{$dataRow}", $statusMap[$detail['blinking_light'] ?? ''] ?? $detail['blinking_light']);
                    $sheet->setCellValue("F{$dataRow}", $statusMap[$detail['connection'] ?? ''] ?? $detail['connection']);
                    $sheet->setCellValue("G{$dataRow}", $statusMap[$detail['audiobility'] ?? ''] ?? $detail['audiobility']);
                    $sheet->setCellValue("H{$dataRow}", $statusMap[$detail['condition_of_hooter'] ?? ''] ?? $detail['condition_of_hooter']);
                    $sheet->mergeCells("I{$dataRow}:K{$dataRow}")->setCellValue("I{$dataRow}", $detail['remarks'] ?? '');
    ...
    public function GeneralExcel(Request $request)
    {
        try {
            ...
            $inspection = $this->hooter->selectOne($id);
            $inspection_data = $this->hooter_details->GetDetails($inspection->id);
            $document_no = $this->document_reference->selectOne($inspection->document_reference_id);
            ...
            $sheet->setCellValue("C1", "HOOTER  INSPECTION CHECKLIST");
            ...
            $sheet->setCellValue("F7", "BLINKING LIGHT");
            $sheet->setCellValue("G7", "CONNECTION");
            $sheet->setCellValue("H7", "AUDIOBILITY");
            $sheet->setCellValue("I7", "CONDITION OF HOOTER");
            ...
            foreach ($inspection_data as $detail) {
                ...
                $sheet->setCellValue("E$row", $statusMap[$detail['quantity'] ?? ''] ?? $detail['quantity'] ?? '');
                $sheet->setCellValue("F$row", $statusMap[$detail['blinking_light'] ?? ''] ?? $detail['blinking_light'] ?? '');
                $sheet->setCellValue("G$row", $detail['connection'] ?? '');
                $sheet->setCellValue("H$row", $statusMap[$detail['audiobility'] ?? ''] ?? $detail['audiobility'] ?? '');
                $sheet->setCellValue("I$row", $statusMap[$detail['condition_of_hooter'] ?? ''] ?? $detail['condition_of_hooter'] ?? '');
            ...
            $preparedBy = getUsername($inspection->created_by);
            $verifiedBy = getUsername($inspection->verified_by);
            $approvedBy = getUsername($inspection->approved_by);
            ...
            foreach (['A', 'E', 'I'] as $col) {
                $sheet->getStyle("{$col}{$signatureRowStart}:" . chr(ord($col) + 3) . "{$signatureRowStart}")->applyFromArray([
                    'borders' => ['allBorders' => ['borderStyle' => Border::BORDER_THIN]],
                    'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER, 'vertical' => Alignment::VERTICAL_CENTER, 'wrapText' => true],
                ]);
            }

            $richPrepared = new RichText();
            $richPrepared->createTextRun("Prepared By : " . ($preparedBy ?: 'Inspection has not been Prepared Yet'))->getFont()->setBold(true);
            $sheet->getCell("A{$signatureRowStart}")->setValue($richPrepared);

            $richVerified = new RichText();
            $richVerified->createTextRun("Verified By : " . ($verifiedBy ?: 'Inspection has not been Verified Yet'))->getFont()->setBold(true);
            $sheet->getCell("E{$signatureRowStart}")->setValue($richVerified);

            $richApproved = new RichText();
            $richApproved->createTextRun("Approved By : " . ($approvedBy ?: 'Inspection has not been Approved Yet'))->getFont()->setBold(true);
            $sheet->getCell("I{$signatureRowStart}")->setValue($richApproved);
