...
class HoseReelHoseController extends Controller
{
    private $hose_reel;
    private $hose_reel_details;
    private $shift;
    private $location;
    private $unit;
    private $frequency;
    ...
    private $hose_reel_type;

    public function __construct()
    {
        $this->hose_reel = new HoseReelHoseInspection();
        $this->hose_reel_details = new HoseReelHoseDetails();
        ...
        $this->hose_reel_type = new HoseBoxType();
    }
    ...
    public function Index(Request $request)
    {
                ...
                try {
                    ...
                    $datatables = DataTables::of($data['data'])
                        ...
                        ->addColumn('action', function ($row) {
                            $btn = '';
                            $btn = '<a href="' . admin_url('fire/hose-reel-hose-inspection/view/' . encryptId($row->inspection_id)) . '"   class="view-icon me-1" title="' . __('common.view') . '"><i class="fa-solid fa-eye"></i></a> ';
                            if ($row->inspection_status == WAITING_FOR_EHS_OFFICER_VERIFICATION && (CheckUserRole(ROLE_EHS_OFFICER) || isAdmin())) {
                                $btn .= '<a href="' . admin_url('fire/hose-reel-hose-inspection/verification/' . encryptId($row->inspection_id)) . '/ehs" class="me-1" title="' . __('inspection.ehs_officer_verify') . '"><i class="fa-solid fa-check-to-slot text-success"></i></a> ';
                            }
                            if (($row->inspection_status == WAITING_FOR_CAPA_ACTION || $row->inspection_status == L2_MANAGER_REJECTED || $row->inspection_status == EHS_OFFICER_REJECTED || $row->inspection_status == L1_MANAGER_REJECTED) && (CheckUserRole(ROLE_FIRE_ASSOCIATES) || isAdmin())) {
                                $btn .= '<a href="' . admin_url('fire/hose-reel-hose-inspection/verification/' . encryptId($row->inspection_id)) . '/capa" class="me-1" title="' . __('inspection.capa_action') . '"><i class="fa-solid fa-check-to-slot text-success"></i></a> ';
                            }
                            if ($row->inspection_status == WAITING_FOR_CAPA_VERIFICATION && (CheckUserRole(ROLE_EHS_OFFICER) || isAdmin())) {
                                $btn .= '<a href="' . admin_url('fire/hose-reel-hose-inspection/verification/' . encryptId($row->inspection_id)) . '/ehsVerify" class="me-1" title="' . __('inspection.ehs_officer_verify') . '"><i class="fa-solid fa-check-to-slot text-success"></i></a> ';
                            }
                            if ($row->inspection_status == WAITING_FOR_L1_VERIFICATION && (CheckUserRole(ROLE_L1_MANAGER) || isAdmin())) {
                                $btn .= '<a href="' . admin_url('fire/hose-reel-hose-inspection/verification/' . encryptId($row->inspection_id)) . '/level-one-manager" class="me-1" title="' . __('inspection.l1_manager_verify') . '"><i class="fa-solid fa-check-to-slot text-success"></i></a> ';
                            }
                            if ($row->inspection_status == WAITING_FOR_L2_VERIFICATION && (CheckUserRole(ROLE_L2_MANAGER) || isAdmin())) {
                                $btn .= '<a href="' . admin_url('fire/hose-reel-hose-inspection/verification/' . encryptId($row->inspection_id)) . '/level-two-manager" class="me-1" title="' . __('inspection.l2_manager_verify') . '"><i class="fa-solid fa-check-to-slot text-success"></i></a> ';
                            }
                            $btn .= '<a href="' . admin_url('fire/hose-reel-hose-inspection/exportViewPdf/' . encryptId($row->inspection_id)) . '" style="margin-right: 5px;" title="PDF">
        ...
        return view('inspection.fire.hose_reel.list', $data);
    }
    ...
    public function Add(Request $request)
    {
        try {
            ...
            $document_no = $this->document_reference->selectUsingName('HoseReelHoseInspection');
            $types = $this->hose_reel_type->getTypes();
            ...
            return view('inspection.fire.hose_reel.add', $data);
        } catch (Exception $ex) {
            report($ex);
    ...
    public function Store(Request $request)
    {
        try {
            ...
            $inspection = $this->hose_reel->store();
    ...
    public function View(Request $request)
    {
        try {
            ...
            $inspection = $this->hose_reel->selectOne($id);
            $inspection_details = $this->hose_reel_details->GetDetails($inspection->id);
            ...
            return view('inspection.fire.hose_reel.view', $data);
        } catch (Exception $ex) {
            report($ex);
            Session::flash('error', 'Something went wrong !');
            return redirect(admin_url('fire/hose-reel-hose-inspection/list'));
    ...
    public function Approvals(Request $request)
    {
        try {
            ...
            return view('inspection.fire.hose_reel.approve', $data);
        } catch (Exception $ex) {
            report($ex);
            Session::flash('error', 'Something went wrong !');
            return redirect(admin_url('fire/hose-reel-hose-inspection/list'));
    ...
    public function EHSOfficerSubmit(Request $request)
    {
        ...
        try {
            $id = decryptId($request->id);
            $inspection_updates = $this->hose_reel->EHSOfficerUpdate($id);
            $signature_update = $this->signature->signatureUpload(HOSE_REEL_INSPECTION);
            $inspection_details = $this->hose_reel->selectOne($id);
            if ($request->is_passed == 1) {
                $message = 'Hose Reel Inspection Approved Successfully';
                $web_link =   admin_url('fire/hose-reel-hose-inspection/verification/' . encryptId($inspection_details->id));
            ...
            $email_id = getUseremail($user);
            $url = admin_url('fire/hose-reel-hose-inspection/verification/' . encryptId($id) . '/capa');
            $details = array(
                'fire_type' => 'Hose Reel Inspection',
                'email' => $email_id,
                'mail_subject' => $mailsubject,
    ...
    public function CAPASubmit(Request $request)
    {
        try {
            $id = decryptId($request->id);
            $hose_reel_inspection = $this->hose_reel->capaSubmit($id);
            $inspection_details = $this->hose_reel->selectOne($id);
            $signature_update = $this->signature->signatureUpload(HOSE_REEL_INSPECTION);
            ...
            $notificationData = array(
                ...
                'web_link' =>  admin_url('fire/hose-reel-hose-inspection/verification/' . encryptId($inspection_details->id)) . '/ehsVerify',
                'assigned_user' => array_to_string($userIds),
                'created_by' => Auth::id(),
            );
            notificationSave($notificationData);
    ...
    public function CAPAVerifySubmit(Request $request)
    {
        try {
            ...
            $remarks = $request->remarks;
            $hose_reel_inspection = $this->hose_reel->capaVerifySubmit($id, $status, $remarks);
            $signature_update = $this->signature->signatureUpload(HOSE_REEL_INSPECTION);
            $inspection_details = $this->hose_reel->selectOne($id);
            if ($status == 1) {
                $message = 'CAPA Action Verified Successfully';
                $web_link =   admin_url('fire/hose-reel-hose-inspection/verification/' . encryptId($inspection_details->id) . '/level-one-manager');
    ...
    public function levelOneManagerSubmit(Request $request)
    {
        try {
            ...
            $hose_reel_inspection = $this->hose_reel->levelOneManagerSubmit($id, $status, $remarks);
            $signature_update = $this->signature->signatureUpload(HOSE_REEL_INSPECTION);
            $inspection_details = $this->hose_reel->selectOne($id);
            if ($status == 1) {
                $message = 'Level One Manager Verified Successfully';
                $web_link =   admin_url('fire/hose-reel-hose-inspection/verification/' . encryptId($inspection_details->id) . '/level-two-manager');
    ...
    public function levelTwoManagerSubmit(Request $request)
    {
        try {
            ...
            $hose_reel_inspection = $this->hose_reel->levelTwoManagerSubmit($id, $status, $remarks);
            $signature_update = $this->signature->signatureUpload(HOSE_REEL_INSPECTION);
            $inspection_details = $this->hose_reel->selectOne($id);
            if ($status == 1) {
                $message = 'Hose Reel Inspection Approved Successfully!';
                $web_link =   admin_url('fire/hose-reel-hose-inspection/view/' . encryptId($inspection_details->id));
                $to_status = INSPECTION_APPROVED;
    ...
    public function ExportExcel(Request $request)
    {
        try {
            ...
            foreach ($allData as $groupedDetails) {
                ...
                $prepared_by_signature = GetFireSignature($inspection_detail->checked_by, $inspection_detail->fire_id, HOSE_REEL_INSPECTION);
                $verified_by_signature = GetFireSignature($inspection_detail->verified_by, $inspection_detail->fire_id, HOSE_REEL_INSPECTION);
                $approved_by_signature = GetFireSignature($inspection_detail->approved_by, $inspection_detail->fire_id, HOSE_REEL_INSPECTION);
                ...
                $sheet->setCellValue("E" . ($headerStart + 1), "NOZZLE CONDITION");
                $sheet->setCellValue("F" . ($headerStart + 1), "HOSE CONDITION");
                $sheet->setCellValue("G" . ($headerStart + 1), "FLOW TEST");
                $sheet->setCellValue("H" . ($headerStart + 1), "STATUS");
                ...
                foreach ($groupedDetails as $detail) {
                    ...
                    $hose = $detail['hose'] ?? '';
                    $flow = $detail['flow'] ?? '';
                    $status_of_hose = $detail['status_of_hose'] ?? '';
                    ...
                    $pass_map = [PASS => 'Pass', FAIL => 'Fail'];

                    $operation_map = [OPERATIONAL => 'Operational', NONOPERATIONAL => 'Non Operational'];

                    $sheet->setCellValue("D$dataRow", $length);
                    $sheet->setCellValue("E$dataRow", $good_map[$nozzle] ?? $nozzle);
                    $sheet->setCellValue("F$dataRow", $good_map[$hose] ?? $hose);
                    $sheet->setCellValue("G$dataRow", $pass_map[$flow] ?? $flow);
                    $sheet->setCellValue("H$dataRow", $operation_map[$status_of_hose] ?? $status_of_hose);
                ...
                if ($inspection_detail->verified_by != null) {
                    $sheet->setCellValue("E{$signatureRowStart}", "Verified By:" . getUsername($inspection_detail->verified_by));
                } else {
                    $sheet->setCellValue("E{$signatureRowStart}", "Verified By: Inspection not yet completed");
                }
                ...
                if ($inspection_detail->approved_by != null) {
                    $sheet->setCellValue("H{$signatureRowStart}", "Approved By: " . getUsername($inspection_detail->approved_by));
                } else {
                    $sheet->setCellValue("H{$signatureRowStart}", "Approved By: Approval pending");
                }
            ...
            $fileName = 'Hose Reel Hose Inspection.xlsx';
    ...
    public function ExportViewPDF(Request $request)
    {
        try {
            ...
            $html = view('inspection.fire.hose_reel.viewPdf', $data);
            $view = $html->render();
            $mpdf->WriteHTML($view);
    ...
    public function GeneralExcel(Request $request)
    {
        try {
            ...
            $inspection         = $this->hose_reel->selectOne($id);
            $inspection_data    = $this->hose_reel_details->GetDetails($inspection->id);
            $document_no        = $this->document_reference->selectOne($inspection->document_reference_id);
            ...
            $sheet->setCellValue("F7", "HOSE CONDITION");
            $sheet->setCellValue("G7", "FLOW TEST");
            ...
            foreach ($inspection_data as $detail) {
                ...
                $sheet->setCellValue("D$row", $length);
                $sheet->setCellValue("E$row", $good_map[$nozzle] ?? $nozzle);
                $sheet->setCellValue("F$row", $good_map[$hose] ?? $hose);
                $sheet->setCellValue("G$row", $pass_map[$flow] ?? $flow);
                $sheet->setCellValue("H$row", $operation_map[$status_of_hose] ?? $status_of_hose);
            ...
            if ($inspection->verified_by != null) {
                $sheet->setCellValue("E{$signatureRowStart}", "Verified By:" . getUsername($inspection->verified_by));
            } else {
                $sheet->setCellValue("E{$signatureRowStart}", "Verified By: Inspection not yet completed");
            }
            ...
            if ($inspection->approved_by != null) {
                $sheet->setCellValue("I{$signatureRowStart}", "Approved By:" . getUsername($inspection->approved_by));
            } else {
                $sheet->setCellValue("I{$signatureRowStart}", "Approved By: Approval pending");
            }
