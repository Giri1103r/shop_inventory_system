...
@section('title', '6S Audit Analysis List')
   ...
   <input type="hidden" name="company_name" value="{{$companyId}}">
                                        ...
                                        <div class="col-md-4 mb-3 form-input">
                                            ...
                                            <select name="audit_analysis_id" id="audit_analysis_id"
                                                ...
                                                <option value="">Selecte 6S Audit Analysis</option>
                                                @foreach ($auditAnalysisList as $list)
                                                    <option value="{{ encryptId($list->id) }}">
                                                        {{ $list->audit_analysis_id }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-4 mb-3 form-input">
    ...
    <script type="text/javascript">
        ...
        $(function() {
            ...
            $(document).on('click', '.statusChange', function() {
                ...
                if (types == 1) {
                    var title = '{{ __('Do You want to In-Activate Detail') }}';
                    var text = '{{ __('common.inactive') }}';
                    var btncolor = '#dc3545'

                } else {
                    var title = '{{ __('Do You want to Activate Detail') }}';
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ admin_url('audit/6s-analysis/status') }}",
                            type: 'post',
            ...
            $(document).on('click', '.recordDelete', function() {
                ...
                var title = '{{ __('Do You want to Delete Detail') }}';
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ admin_url('audit/6s-analysis/delete') }}",
                            type: 'post',
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]')
                                    .attr('content')
